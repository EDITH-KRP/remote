...
const statConfig = [
  { key: "total_tickets",      label: "Total",       icon: Ticket,    g: "linear-gradient(135deg,#6366f1,#4f46e5)", glow: "0 6px 24px rgba(99,102,241,0.45)", glowBg: "rgba(99,102,241,0.1)", accent: "rgba(99,102,241,0.3)" },
  ...
  { key: "inprogress_tickets", label: "In Progress", icon: Activity,  g: "linear-gradient(135deg,#8b5cf6,#7c3aed)", glow: "0 6px 24px rgba(139,92,246,0.4)", glowBg: "rgba(139,92,246,0.08)", accent: "rgba(139,92,246,0.25)" },
  ...
  return (
    <div style={{ maxWidth: "1280px", margin: "0 auto", paddingBottom: "4rem" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem", marginBottom: "1.375rem" }}>
        <div style={{ display: "inline-flex", gap: "4px", padding: "4px", background: "rgba(9,11,22,0.85)", border: "1px solid rgba(99,102,241,0.14)", borderRadius: "var(--r-md)", backdropFilter: "blur(16px)" }}>
          {[
            ...
            <button 
              ...
              style={{ 
                ...
                background: tab === t.id ? "linear-gradient(135deg,rgba(99,102,241,0.22),rgba(139,92,246,0.14))" : "transparent", 
                color: tab === t.id ? "var(--p3)" : "var(--text-2)" 
              }}
            >
              {t.icon} {t.label}
              {t.count !== undefined && (
                <span style={{ padding: "1px 7px", borderRadius: "99px", fontSize: "0.7rem", fontWeight: 800, background: tab === t.id ? "rgba(99,102,241,0.18)" : "rgba(255,255,255,0.05)", color: tab === t.id ? "var(--p3)" : "var(--text-3)" }}>
                  {t.count}
                </span>
              )}
            </button>
        ...
        {tab === "dashboard" && (
          <motion.div key="dashboard" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.22 }} style={{ display: "flex", flexDirection: "column", gap: "1.75rem" }}>
                ...
                {statConfig.map(s => {
                  ...
                  return (
                    <motion.div 
                      ...
                      style={{ 
                        background: "rgba(10,12,22,0.82)", border: "1px solid rgba(99,102,241,0.12)", 
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "1.25rem" }} className="lg:grid-cols-dash">
              ...
              <div style={{ background: "rgba(10,12,22,0.82)", border: "1px solid rgba(99,102,241,0.12)", borderRadius: "var(--r-lg)", backdropFilter: "blur(20px)", overflow: "hidden" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1.1rem 1.5rem", borderBottom: "1px solid rgba(99,102,241,0.09)", background: "rgba(99,102,241,0.03)" }}>
              ...
              {/* Weekly Trend Chart */}
              <div style={{ background: "rgba(10,12,22,0.82)", border: "1px solid rgba(99,102,241,0.12)", borderRadius: "var(--r-lg)", backdropFilter: "blur(20px)", padding: "1.25rem 1.5rem 1rem" }}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={activityData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="created"  stroke="#6366f1" strokeWidth={2} fill="url(#gCreatedAdmin)"  dot={{ fill: "#6366f1", r: 3, strokeWidth: 0 }} activeDot={{ r: 5 }} />
      ...
      {/* ── Side Panel (ServiceNow-Aligned Detail Form) ── */}
      <AnimatePresence>
        {selectedId && selectedTicket && detailedTicket && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(3px)", display: "flex", justifyContent: "flex-end" }} onClick={() => setSelectedId(null)}>
            <motion.div
              ...
              style={{ background: "#0a0c16", borderLeft: "1px solid rgba(99,102,241,0.2)", width: "100%", maxWidth: "560px", height: "100%", overflowY: "auto", padding: "1.75rem", display: "flex", flexDirection: "column", gap: "1.25rem", position: "relative", boxShadow: "-10px 0 40px rgba(0,0,0,0.5)" }}
            >
              ...
              {/* ServiceNow Incident Navigation Breadcrumb Header */}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem 1.25rem", padding: "1rem", background: "rgba(255,255,255,0.015)", border: "1px solid rgba(99,102,241,0.06)", borderRadius: "var(--r-sm)" }}>
              ...
              {/* ServiceNow Full Width Incident Descriptions */}
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                
                <div style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "8px", padding: "0.85rem" }}>
                ...
                <div style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "8px", padding: "0.85rem" }}>
                  ...
                  <p style={{ fontSize: "0.85rem", color: "var(--text-2)", lineHeight: 1.6, whiteSpace: "pre-wrap", marginTop: "4px" }}>{detailedTicket.description}</p>
                </div>

                {detailedTicket.note && (
                  <div style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.04)", borderRadius: "8px", padding: "0.85rem" }}>
                    <p style={serviceNowLabelStyle}>Notes / Additional Info</p>
                    <p style={{ fontSize: "0.825rem", color: "var(--text-3)", lineHeight: 1.5, marginTop: "4px" }}>{detailedTicket.note}</p>
                  </div>
                )}
                ...
                {detailedTicket.attachment_url && (
                  <div style={{ display: "flex", alignItems: "center", gap: "6px", padding: "4px 8px" }}>
                    ...
                    <a href={`${API_URL.replace("/api", "")}${detailedTicket.attachment_url}`} target="_blank" rel="noreferrer" style={{ fontSize: "0.85rem", color: "var(--p3)", textDecoration: "none" }}>
              ...
              <div style={{ padding: "0.85rem", background: "rgba(99,102,241,0.03)", border: "1px solid rgba(99,102,241,0.1)", borderRadius: "8px", display: "flex", flexDirection: "column", gap: "8px" }}>
                <p style={serviceNowLabelStyle}>ITIL Ticket Routing</p>
              ...
              {/* ServiceNow Incident Activity Stream (Conversation & Audit logs) */}
              <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "1rem", display: "flex", flexDirection: "column", gap: "10px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  ...
                  <span style={{ fontSize: "0.7rem", fontWeight: 700, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                    Incident Activity Stream ({detailedComments.length})
                  </span>
                </div>

                {/* Comment Feed list */}
                <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", maxH: "200px", overflowY: "auto", paddingRight: "4px" }}>
                  {detailedComments.map(c => (
                    <div 
                      key={c.id} 
                      style={{ 
                        background: c.is_internal ? "rgba(245,158,11,0.05)" : (c.author_id === detailedTicket.user_id ? "rgba(99,102,241,0.05)" : "rgba(16,185,129,0.05)"),
                        border: `1px solid ${c.is_internal ? "rgba(245,158,11,0.2)" : (c.author_id === detailedTicket.user_id ? "rgba(99,102,241,0.2)" : "rgba(16,185,129,0.2)")}`,
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.35rem" }}>
                        <span style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--text-1)" }}>
                          {c.author?.full_name} 
                          {c.is_internal && <span style={{ marginLeft: "6px", fontSize: "0.625rem", color: "#f59e0b", background: "rgba(245,158,11,0.15)", padding: "1px 5px", borderRadius: "3px" }}><Lock size={9} style={{ display:"inline", marginRight:"2px" }}/> Work Note</span>}
                        </span>
                      ...
                      <p style={{ fontSize: "0.81rem", color: "var(--text-2)", whiteSpace: "pre-wrap", lineHeight: 1.4 }}>{c.body}</p>
                    </div>
                  ))}
                ...
                {/* Reply Form (Additional Comments or Internal Work Notes) */}
                  ...
                  <form onSubmit={handleAddComment} style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "4px" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <label style={{ display: "flex", alignItems: "center", gap: "6px", cursor: "pointer", fontSize: "0.75rem", color: "var(--text-2)" }}>
                        ...
                        />
                        <span>Work Note (Internal Comment)</span>
                      </label>
              ...
              {/* Audit logs of ticket activity */}
              {detailedTicket.logs && detailedTicket.logs.length > 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: "8px", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "1rem" }}>
                  <p style={serviceNowLabelStyle}>Audit History Log</p>
                  <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    {detailedTicket.logs.map(log => (
                      <div key={log.id} style={{ display: "flex", gap: "6px", alignItems: "center", background: "rgba(255,255,255,0.01)", border: "1px solid rgba(255,255,255,0.03)", borderRadius: "4px", padding: "4px 8px" }}>
                        <Clock size={11} style={{ color: "var(--text-3)" }} />
                        <span style={{ fontSize: "0.75rem", color: "var(--text-2)" }}>
                          <strong>{log.action}</strong> at {new Date(log.createdAt || log.timestamp).toLocaleDateString()} {new Date(log.createdAt || log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "1.25rem", display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "auto" }}>
